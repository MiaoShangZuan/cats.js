...
/**
 * 任意贝塞尔曲线轨道
 * @param {object} points 三次贝塞尔曲线顶点数组
 * @param {number} start_time 开始时间
 * @param {number} time 持续时间
 * @returns 返回三个AnFunc
 */
function AnGetBezierFunc( points, start_time, time ) {
    ...
    return [
        ...
        new AnFunc( start_time, end_time, t=>{t/=time;const dy = 3 * ay * t * t + 2 * by * t + cy, dx = 3 * ax * t * t + 2 * bx * t + cx, angle = dy / sqrt( dx * dx + dy * dy );return _sign(dx) * acos(angle);})
    ];
}
...
class Animation {
            ...
            if( animation.state == 'play' ) {
                ...
                if( animation.t <= animation.time ) {
                    ...
                    animation.keys.forEach(
                        key => {
                            animation.__attributes[key] = animation.__controller[key].value(tis);
                        }
                    );
    ...
    play() {
        if( this.state != 'play' ) {
            ...
            this.keys.forEach(
                key => {
                    this.__attributes[key] = this.__controller[key].value(tis);
                }
            );
...
export { AnBezier, AnFunc, AnLerp, AnCureController, Animation, AnGetBezierFunc };